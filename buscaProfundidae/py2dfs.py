...
def dfs_visual(grafo_dict, fonte):
    ...
    def desenhar(titulo):
        ax.clear()
        # Mapeamento: white (Branco), gray (Cinza), #FFFFE0 (Amarelo Claro)
        ...
        nx.draw(G, pos, with_labels=True, node_color=lista_cores, 
                ...
                arrowsize=20, edgecolors="black") # Borda preta para destacar nós amarelos
        plt.title(titulo)
        plt.pause(1.0)
    ...
    def dfs_visit(u):
        ...
        tempo += 1
        # ALTERAÇÃO AQUI: Troca de 'black' por Amarelo Claro
        cores[u] = "#FFFFE0" 
    ...
    plt.ioff()
    plt.title("DFS Concluída - Nós finalizados em Amarelo Claro")
    plt.show(block=True)
